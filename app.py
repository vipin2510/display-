...
TEMPLATE_SPREADSHEET_ID = os.environ.get('TEMPLATE_SPREADSHEET_ID', '19SKGC7kUFi3ZVZEHxSavCrGOMuHVQASs-3jTIddFedg')
...
def get_settings(spreadsheet_id):
    ...
    settings = sheet.get_all_values()[1:]
    return [
        {
            'sheet_name': row[0],
            'time_of_display': int(row[1]),
            'columns_to_display': row[3].split(','),
            'photo_column': row[6] if row[6] else None,
            'display': row[4],
            'title': row[5]
        }
        for row in settings if row[1] != '0'
    ]
...
def check_changes():
    try:
        ...
        logging.error(f"An error occurred in check_changes: {str(e)}")
